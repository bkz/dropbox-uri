...
_create_shortcut = CDLL("platform").create_shortcut
...
_is_admin = CDLL("platform").is_admin
_is_admin.restype = c_int
_is_admin.argtypes = []

###########################################################################
# Lookup platform specific system folders.
###########################################################################
...
def get_dropbox_dbfile(filename):
    """
    Locaate Dropbox SQLite database ``filename`` in platform specific
...
if sys.platform == "win32":
    def install(rootdir, is_frozen, script_path=None):
        sendto = get_system_path(CSIDL_SENDTO)
        create_shortcut(
